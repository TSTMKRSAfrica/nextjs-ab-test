...
function Experiment({ children, name, activeVariant, weights = [50, 50], onExperimentLoad = () => console.log("Experiment Loaded") }){
      ...
      <ExperimentStateContext.Provider value={state}>
        {
          !state.activeVariant && !activeVariant ?
            ...
            :
            <>
							{activeVariant && children.find(child => child.props.name == activeVariant)}
              {state.activeVariant && children.find(child => child.props.name == state.activeVariant)}
            </>
        }
